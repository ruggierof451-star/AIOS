...
async function bootstrap() {
  ...
  // Validazione automatica dei DTO (class-validator) — rifiuta input non
  // conforme prima ancora che raggiunga il livello applicativo, coerente
  // con "validazione di schema sempre prima della validazione di business
  // rule" (Runtime, sezione 4.3).
  app.useGlobalPipes(
    new ValidationPipe({
      whitelist: true,
  ...
  console.log(`[identity] servizio in ascolto sulla porta ${port}`);
}

bootstrap().catch((err) => {
  console.error('[identity] Errore fatale all\'avvio:', err);
  process.exit(1);
});
